...
function App() {
    ...
    const [timeRemaining, setTimeRemaining] = useState(5) // starting from 5s then counts down. 
    ...
    useEffect(() => {
        if(timeRemaining > 0) {
            setTimeout(() => {
                setTimeRemaining(time => time - 1) // time here can be called anything since it's a param
            }, 1000)
        }
    }, [timeRemaining])
